...
The `ojoRegex` package is a collection of regular expressions that can be used to clean and extract information from court data, in particular data from the Oklahoma State Court Network (OSCN). This appendix provides a list of how every unique charge description in the OSCN data is cleaned by the `ojoRegex` package.
...
This appendix covers charge descriptions from the OSCN data which were filed in felony, misdemeanor, or traffic cases from `r start_year` to `r end_year`. There are `r nrow(ds_clean) |> format(big.mark = ",")` total unique charge descriptions in the data, but only those appearing at least 1,000 times from `r start_year` - `r end_year` (`r ds_clean |> filter(n >= 1000) |> nrow() |> format(big.mark = ",")` unique descriptions) are included for brevity. For a full list, please contact [Andrew Bell at abell@okpolicy.org](mailto:abell@okpolicy.org).
