  ...
  result <- ojo_regex_cats |>
    dplyr::mutate(
      max_sentence_first_offense_type = dplyr::case_when(
        ...
        stringr::str_detect(max_sentence_first_offense, "(?i)year") ~
          "Time in DOC",
        stringr::str_detect(max_sentence_first_offense, "(?i)month") ~
          "Time in DOC",
        stringr::str_detect(max_sentence_first_offense, "(?i)week") ~
          "Time in DOC",
        stringr::str_detect(max_sentence_first_offense, "(?i)day") ~
          "Time in DOC",
      ...
      control_rank = dplyr::case_when(
        ...
        max_sentence_first_offense_type == "Time in DOC" ~
          (max_sentence_first_time_in_doc / lubridate::years(1)) |> round(2),
      ...
      control_rank = dplyr::case_when(
        stringr::str_detect(
          ...
          "(?i)murder|manslau|homicid"
        ) ~
          control_rank + 0.5,
        stringr::str_detect(clean_charge_description, "(?i)rape|sex|molest") ~
          control_rank + 0.4,
        stringr::str_detect(clean_charge_description, "(?i)child") ~
          control_rank + 0.3,
        stringr::str_detect(
          clean_charge_description,
          "(?i)aggrivated|armed|weapon"
        ) ~
          control_rank + 0.2,
        TRUE ~ control_rank
      )
